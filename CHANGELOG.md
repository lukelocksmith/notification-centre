...
- **Grupa rotacyjna.** Powiadomienia z tą samą nazwą grupy nie pokazują się jedno po drugim: z grupy wyświetla się najwyżej jedno (to, które najdawniej było pokazane), ze wspólnym limitem 3 wyświetleń na 7 dni i przerwą 24 h (filtr `nc_rotation_limits`).
### Performance
- **Zero kodu wtyczki przy ładowaniu strony.** `main.js` i `style.css` nie są już dołączane jako znaczniki. Mały skrypt w stopce (ok. 0,5 KB) doczytuje je przy pierwszym ruchu myszy, dotknięciu, klawiszu, kółku albo przewinięciu. PageSpeed Insights i roboty wyszukiwarek nie wchodzą w interakcję ze stroną, więc nie pobierają nic z wtyczki; zapytanie do REST też idzie dopiero po interakcji. W stronie zostają tylko dane `ncData`, zmienne CSS i (gdy jest) pasek z serwera ze stylami w linii. Licznik w pasku odlicza bez `main.js`. Kliknięcie w dzwonek przed doczytaniem jest odtwarzane.
- **Popupy automatyczne nie psują już LCP.** Popup z opóźnieniem, czasem na stronie, bezczynnością albo bez wyzwalacza pokazuje się dopiero po pierwszej interakcji (dotknięcie, klik, klawisz, kółko myszy). Wcześniej obraz popupu na środku ekranu stawał się największym elementem strony: na wpisie bloga WDF LCP 13,1 s zamiast 5,5 s. Popup pojawia się 0,8 s po tej interakcji, a nie w jej trakcie, żeby nie wyskoczył pod palcem i nie przejął tego samego kliknięcia. Exit intent i klik w selektor działają bez zmian.
- **Pasek górny renderowany przez serwer** (paski bez podziału na urządzenia, bez „tylko klienci” i grup rotacyjnych; paski z krzyżykiem też, a zamknięte wcześniej w tej przeglądarce skrypt przy pasku usuwa przed pierwszym renderem): stoi na miejscu od pierwszego wyrenderowania, więc treść nie skacze w dół (CLS). Na Bricks trafia od razu pod nagłówek (`bricks_after_header`). Style paska są wstawiane w linii tylko na stronach z paskiem. Czas życia cache strony w LiteSpeed jest skracany do najbliższej granicy harmonogramu (np. 10:00), a skrypt w linii ukrywa pasek, którego godzina graniczna już minęła.
- **Fluent Forms tylko wtedy, gdy potrzebny.** Pliki Fluent Forms nie są już dołączane do każdej strony; `main.js` doczytuje je tuż przed pokazaniem popupu z formularzem.
- **`GET /nc/v1/notifications` cache'owalne dla gości.** Front wysyła klasę urządzenia (`d`) i wersję (`v`), usuwa z adresu identyfikatory reklam (gclid, fbclid, ...). Z `d` odpowiedź dla gości idzie do cache LiteSpeed (TTL do 5 min albo do najbliższej granicy harmonogramu); bez `d` (stary `main.js`) zostaje `no-cache`.
- **Zdarzenia analityki w paczce**: jeden `POST /nc/v1/events` na stronę (przy ukryciu karty, wyjściu albo po 10 s) zamiast osobnego zapytania na każde zdarzenie. Stary format pojedynczego zdarzenia nadal działa.
- **Koniec czyszczenia całego cache LiteSpeed przy każdym zapisie powiadomienia.** Cache stron jest czyszczony tylko, gdy zmienia się coś, co jest w HTML: opublikowany pasek albo lista formularzy. Pasek z regułami „pokaż na stronie o ID” / „strona główna” czyści tylko te strony (tagi `Po.<ID>`, `F`), pasek na całą stronę czyści całość. Edycja popupu, szkic i autozapis już nie czyszczą cache sklepu.
- `GET /nc/v1/user-notifications` tylko gdy powiadomienia WooCommerce są włączone (`nc_woo_enabled`).
### Fixed
- Baner z licznikiem dziennym i autoukrywaniem („Zamów do 10:00”) był widoczny jeszcze kilka minut po godzinie granicznej, z licznikiem ustawionym na jutro (porównanie `>` zamiast `>=`, transient na 5 min). Teraz znika dokładnie o tej godzinie.
- Autoukrycie licznika usuwało cały pasek (razem z innymi paskami) albo zostawiało pustą nakładkę popupu i blokowało kolejkę. Teraz znika tylko ten jeden element, a popup zamyka się normalnie.
- Niepoprawny selektor w wyzwalaczu „klik” rzucał wyjątek przy każdym kliknięciu na stronie.
- **Formularz z reCAPTCHA w popupie nie dawał się wysłać** (`grecaptcha is not defined`): Fluent Forms dołącza skrypt Google podczas renderowania formularza, a treść popupu powstaje w zapytaniu REST, gdzie ten skrypt nigdy nie trafiał na stronę. REST przekazuje teraz jego adres (`scripts`), `main.js` ładuje go przed pokazaniem popupu. Na WDF dotyczyło to popupu z ebookiem (formularz 24).
- Kolumna „Odbiorcy” na liście powiadomień pokazywała surowe `guests`.
- Podgląd `?nc_preview=1` znów działa (parametr przekazywany do REST, bez transientu i bez cache).
...
### Added (z linii `main`)
- **Device targeting** — select „Urządzenie" (sekcja Targetowanie) ogranicza powiadomienie do `Tylko mobile (telefony + tablety)` lub `Tylko desktop`, oceniane live per-request przez `wp_is_mobile()`.
- **Twardy limit wyświetleń (Capping)** — min. godziny między pokazami, maks. liczba pokazów i okno (w dniach), liczone niezależnie od tego, czy użytkownik zamknął powiadomienie.
### Fixed (z linii `main`)
- **Powiadomienia device-targeted przeciekały między urządzeniami przez cache** — klucz transientu REST uwzględnia teraz klasę urządzenia, a endpoint zawsze wysyła `no-cache` na edge (LiteSpeed nie waruje po device). Zintegrowane z liczeniem klucza cache po **ścieżce** z tej gałęzi.
- **Zmiany powiadomienia widoczne dopiero po ~5 min** — `nc_cache_version` bumpowany bezpośrednio przy zapisie w wp-admin, niezależnie od tego czy cron istnieje.
### Removed
- Tymczasowy `console.log('NC_DEBUG', …)` z `showFloating()` (dodany w 1.6.3 do diagnozy `hide_title`).
